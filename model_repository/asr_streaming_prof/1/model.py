# ABOUTME: BẢN ĐO - sao của asr_streaming, cắm timer từng tầng để biết thời gian nằm ở đâu
# ABOUTME: Không phải đường phục vụ. Đối chứng là asr_streaming, bản đó không được sửa
...
from serving.metrics import ASR_RTF_BUCKETS, ModelMetrics, StageMetrics  # noqa: E402
...
MODEL_NAME = "asr_streaming_prof"

# "chunk" bọc trọn _handle nên nó ĐÃ CHỨA ba tầng kia - đừng cộng cả bốn vào
# một tổng. Phần chênh chunk - (fbank + encoder + greedy) chính là overhead
# ngoài tầng, và đó là con số cả buổi đo này đi tìm.
STAGES = ["chunk", "fbank", "encoder", "greedy"]
...
class TritonPythonModel:
    def initialize(self, args):
        # Trọng số DÙNG CHUNG với asr_streaming, không sao chép sang đây. Hai lý
        # do: khỏi nhân đôi 51MB trong git, và quan trọng hơn - bản đo phải chạy
        # đúng bộ số của đường đối chứng thì bảng phân rã mới nói được điều gì
        # về đường đối chứng. Sao chép ra là mở cửa cho hai bên lệch nhau âm thầm.
        ...
        self.stages = StageMetrics(pb_utils, MODEL_NAME, STAGES)
    ...
    def _advance(self, stream, new_feat, flush):
        ...
        while stream.feat.shape[0] >= self.T:
            ...
            with self.stages.time("greedy"):
                greedy_search_step(
                    enc_out, stream.search, self.run_decoder, self.run_joiner,
